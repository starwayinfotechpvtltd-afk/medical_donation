...
export default function LabTechLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(email, password, 'lab_technician');
      if (result.success) {
        router.push(getDashboardPathForRole('lab_technician'));
        return;
      }
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/labtech/request-otp', { email });
      setSetupStep(2);
      setSuccess('OTP sent to your lab technician email.');
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/labtech/verify-otp', { email, otp });
      setSetupStep(3);
  ...
  const setLabTechPassword = async () => {
    clearMessages();
    setLoading(true);
    try {
      await api.post('/auth/labtech/set-password', { email, password: newPassword });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Lab Technician Access</h1>
        ...
        <div className="mb-6 inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'login' ? 'bg-purple-600 text-white' : 'text-slate-700'}`}
          >
            Login
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'setup' ? 'bg-purple-600 text-white' : 'text-slate-700'}`}
          >
            First Time Setup
          </button>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-4 py-3 pr-12 outline-none focus:border-purple-500"
                  required
                />
          ...
          <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    placeholder="123456"
                    className="w-full rounded-lg border border-slate-300 px-4 py-3 outline-none focus:border-purple-500"
                  />
                </div>
                <button
            ...
            {setupStep === 3 ? (
              <>
                ...
                <button
                  onClick={() => void setLabTechPassword()}
        ...
        <div className="mt-6 text-center text-sm text-slate-600 space-y-2">
          <Link href="/" className="text-purple-600 hover:text-purple-700 font-medium">Back to Home</Link>
          <div className="flex gap-2 justify-center">
            <Link href="/patient/login" className="text-purple-600 hover:text-purple-700">Patient Login</Link>
            <span>|</span>
            <Link href="/doctorLogin" className="text-purple-600 hover:text-purple-700">Doctor Login</Link>
            <span>|</span>
            <Link href="/adminLogin" className="text-purple-600 hover:text-purple-700">Admin Login</Link>
          </div>
        </div>
      </div>
    </div>
...
function FieldEmail({ email, setEmail }: { email: string; setEmail: (value: string) => void }) {
  return (
    <div>
      <label className="block text-sm font-medium text-slate-700 mb-2">Lab Technician Email</label>
      <input
        type="email"
        value={email}
