...
export default function DoctorAdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(email, password, 'doctor_admin');
      if (result.success) {
        router.push(getDashboardPathForRole('doctor_admin'));
        return;
      }
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/doctor-admin/request-otp', { email });
      setSetupStep(2);
      setSuccess('OTP sent to your doctor admin email.');
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/doctor-admin/verify-otp', { email, otp });
      setSetupStep(3);
  ...
  const setDoctorAdminPassword = async () => {
    clearMessages();
    setLoading(true);
    try {
      await api.post('/auth/doctor-admin/set-password', { email, password: newPassword });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-slate-100 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Doctor Admin Access</h1>
        ...
        <div className="mb-6 inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'login' ? 'bg-indigo-600 text-white' : 'text-slate-700'}`}
          >
            Login
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'setup' ? 'bg-indigo-600 text-white' : 'text-slate-700'}`}
          >
            First Time Setup
          </button>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-4 py-3 pr-12 outline-none focus:border-indigo-500"
                  required
                />
          ...
          <div className="space-y-4">
            ...
            {setupStep === 3 ? (
              <>
                ...
                <button
                  onClick={() => void setDoctorAdminPassword()}
        ...
        <div className="mt-6 text-center text-sm text-slate-600 space-y-2">
          ...
          <div className="flex gap-2 justify-center">
            <Link href="/patient/login" className="text-indigo-600 hover:text-indigo-700">Patient Login</Link>
            <span>|</span>
            <Link href="/doctorLogin" className="text-indigo-600 hover:text-indigo-700">Doctor Login</Link>
            <span>|</span>
            <Link href="/adminLogin" className="text-indigo-600 hover:text-indigo-700">Admin Login</Link>
          </div>
        </div>
      </div>
    </div>
...
function FieldEmail({ email, setEmail }: { email: string; setEmail: (value: string) => void }) {
  return (
    <div>
      <label className="block text-sm font-medium text-slate-700 mb-2">Doctor Admin Email</label>
      <input
        type="email"
        value={email}
