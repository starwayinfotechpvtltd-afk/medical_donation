...
export default function PatientAdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const result = await login(email, password, 'patient_admin');
      if (result.success) {
        router.push(getDashboardPathForRole('patient_admin'));
        return;
      }
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/patient-admin/request-otp', { email });
      setSetupStep(2);
      setSuccess('OTP sent to your patient admin email.');
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/patient-admin/verify-otp', { email, otp });
      setSetupStep(3);
  ...
  const setPatientAdminPassword = async () => {
    clearMessages();
    setLoading(true);
    try {
      await api.post('/auth/patient-admin/set-password', { email, password: newPassword });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 to-slate-100 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Patient Admin Access</h1>
          <p className="mt-2 text-slate-600">Login or complete first-time password setup</p>
        </div>

        <div className="mb-6 grid w-full grid-cols-2 gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1">
          <button
            onClick={() => {
              setMode('login');
              clearMessages();
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
              mode === 'login' ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-700 hover:bg-white'
            }`}
          >
            Login
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
              mode === 'setup' ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-700 hover:bg-white'
            }`}
          >
            First Time Setup
          </button>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-4 py-3 pr-12 text-slate-900 outline-none focus:border-teal-500"
                  required
                />
          ...
          <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    placeholder="123456"
                    className="w-full rounded-lg border border-slate-300 px-4 py-3 text-slate-900 outline-none focus:border-teal-500"
                  />
                </div>
                <button
            ...
            {setupStep === 3 ? (
              <>
                ...
                <button
                  onClick={() => void setPatientAdminPassword()}
        ...
        <div className="mt-6 space-y-2 text-center text-sm text-slate-600">
          <Link href="/" className="font-medium text-teal-600 hover:text-teal-700">Back to Home</Link>
          <div className="flex justify-center gap-2">
            <Link href="/patient/login" className="text-teal-600 hover:text-teal-700">Patient Login</Link>
            <span>|</span>
            <Link href="/doctorLogin" className="text-teal-600 hover:text-teal-700">Doctor Login</Link>
            <span>|</span>
            <Link href="/adminLogin" className="text-teal-600 hover:text-teal-700">Admin Login</Link>
          </div>
        </div>
      </div>
    </div>
...
function FieldEmail({ email, setEmail }: { email: string; setEmail: (value: string) => void }) {
  return (
    <div>
      <label className="mb-2 block text-sm font-medium text-slate-700">Patient Admin Email</label>
      <input
        type="email"
        value={email}
        onChange={(e) => setEmail(e.target.value)}
        placeholder="patient.admin@hospital.com"
        className="w-full rounded-lg border border-slate-300 px-4 py-3 text-slate-900 outline-none focus:border-teal-500"
        required
      />
    </div>
