...
import { Eye, EyeOff, KeyRound, MailCheck, ShieldCheck, HeartPulse } from 'lucide-react';
...
export default function NurseLoginPage() {
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/nurse/request-otp', { email });
      setSetupStep(2);
      setSuccess('OTP sent to your nurse email.');
    } catch (err) {
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/nurse/verify-otp', { email, otp });
      setSetupStep(3);
  ...
  const setNursePassword = async () => {
    clearMessages();
    setLoading(true);
    try {
      await api.post('/auth/nurse/set-password', { email, password: newPassword });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 to-slate-100 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-rose-600 rounded-lg flex items-center justify-center mx-auto mb-4">
            <HeartPulse className="h-8 w-8 text-white" />
          </div>
          <h1 className="text-3xl font-bold text-slate-900">Nurse Access</h1>
        ...
        <div className="mb-6 inline-flex rounded-xl border border-slate-200 bg-slate-50 p-1">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'login' ? 'bg-rose-600 text-white' : 'text-slate-700'}`}
          >
            Login
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium ${mode === 'setup' ? 'bg-rose-600 text-white' : 'text-slate-700'}`}
          >
            First Time Setup
          </button>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-4 py-3 pr-12 outline-none focus:border-rose-500"
                  required
                />
        ...
        <div className="mt-6 text-center text-sm text-slate-600 space-y-2">
          <Link href="/" className="text-rose-600 hover:text-rose-700 font-medium block">Back to Home</Link>
          <div className="flex gap-2 justify-center">
            <Link href="/patient/login" className="text-rose-600 hover:text-rose-700">Patient Login</Link>
            <span>|</span>
            <Link href="/doctorLogin" className="text-rose-600 hover:text-rose-700">Doctor Login</Link>
            <span>|</span>
            <Link href="/adminLogin" className="text-rose-600 hover:text-rose-700">Admin Login</Link>
          </div>
        </div>
      </div>
    </div>
...
function FieldEmail({ email, setEmail }: { email: string; setEmail: (value: string) => void }) {
  return (
    <div>
      <label className="block text-sm font-medium text-slate-700 mb-2">Nurse Email</label>
      <input
        type="email"
        value={email}
